...
impl DriverActions<FixtureSystem> for FixtureSystem {
    ...
    fn stage(&self, _transport: &FixtureSystem, target: &SshTarget) -> Result<(), BackendFailure> {
        ...
        fs::write(artifact.join(BACKLIGHT_RULE_FILE), BACKLIGHT_RULE)
            .map_err(|_| BackendFailure::OperationFailed)?;
        fs::write(
            artifact.join("prior-backlight-metadata"),
            b"schema_version=1\npresent=false\nowner_uid=none\ngroup_gid=none\nmode=none\n",
        )
        .map_err(|_| BackendFailure::OperationFailed)?;
        fs::set_permissions(
        ...
        fs::write(
            ...
            format!(
                ...
                self.sha256(artifact.join(BACKLIGHT_RULE_FILE)),
                self.sha256(artifact.join("prior-backlight-metadata")),
            ),
        )
        .map_err(|_| BackendFailure::OperationFailed)
    }
            ...
            .any(|value| value == "planeradar-driver-accepted-reuse")
        {
            ...
            let exact = arguments.len() == 13
                && [
                    ...
                    format!("backlight_metadata_capability={}", arguments[11]),
                ]
                .iter()
                .all(|row| accepted.lines().any(|line| line == row))
                && arguments[9] == REQUIRED_DRIVER_CAPABILITY
                && arguments[11] == LIFECYCLE_CAPABILITY;
            let artifact = self.path(format!(
                "/usr/lib/hyperpixel2r-kms/{}/{}/{}",
            ...
            let metadata = artifact.join("prior-backlight-metadata");
            let metadata_is_bound = metadata.is_file()
                && accepted.lines().any(|line| {
                    line == format!("prior_backlight_metadata_sha256={}", self.sha256(&metadata))
                });
            return if exact && prior_tryboot_is_bound && metadata_is_bound {
                Ok(Output::success(Vec::new(), Vec::new()))
            } else {
                Err(TransportError::CommandFailed)
            };
...
fn controller_install_resume_reaches_every_real_phase() {
    ...
    *fixture.shared.driver_tool_target.borrow_mut() = None;
    ...
    assert_eq!(exact_manifest.lines().count(), 18);
    assert!(
        exact_manifest
            .lines()
            .any(|row| { row == format!("lifecycle_capability\t{LIFECYCLE_CAPABILITY}") })
    );
    assert!(
        exact_manifest
            .lines()
    ...
    assert!(
        accepted
            .lines()
            .any(|row| { row == format!("backlight_metadata_capability={LIFECYCLE_CAPABILITY}") })
    );
    let metadata = driver_root.join("prior-backlight-metadata");
    let metadata_sha = fixture.sha256(&metadata);
    assert_eq!(
        accepted
            .lines()
            .find_map(|row| row.strip_prefix("prior_backlight_metadata_sha256=")),
        Some(metadata_sha.as_str())
    );
    assert_eq!(
...
fn controller_install_reuses_an_exact_healthy_accepted_driver_without_tryboot_mutation() {
    ...
    fs::write(
        ...
        format!(
            ...
            postconditions.backlight_rule_sha256,
            fixture.sha256(accepted_artifact.join("prior-backlight-metadata")),
        ),
    )
    .expect("accepted driver receipt");
...
fn accepted_driver_receipt_check_keeps_postconditions_out_of_the_shell_program() {
    ...
    assert!(program.contains("receipt_rows=16"));
    ...
    enhanced.lifecycle_capability = Some(LIFECYCLE_CAPABILITY.into());
    let enhanced_command =
        accepted_driver_receipt_command(&enhanced).expect("enhanced accepted receipt command");
    assert_eq!(
        enhanced_command
            .arguments()
            .iter()
            ...
            .map(String::as_str)
            .collect::<Vec<_>>(),
        ["true", LIFECYCLE_CAPABILITY]
    );
    let enhanced_program = &enhanced_command.arguments()[3];
    assert!(enhanced_program.contains("receipt_rows=18"));
    assert!(enhanced_program.contains("prior-backlight-metadata"));
    assert!(enhanced_program.contains("prior_backlight_metadata_sha256"));

    let mut unknown = expected;
    unknown.lifecycle_capability = Some("unknown-lifecycle-v1".into());
    assert!(accepted_driver_receipt_command(&unknown).is_err());
    assert!(recorded_driver_receipt_command(&unknown).is_err());
    for path in [
        "tryboot-state",
        "rollback-state",
        "accepted-transition",
