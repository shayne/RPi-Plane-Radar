...
pub fn accepted_driver_receipt_command(
    ...
    RemoteCommand::interactive_sudo([
        ...
        concat!(
            ...
            r#"test "$(uname -m)" = aarch64; test "$(uname -r)" = "$3"; tryboot_flag=/proc/device-tree/chosen/bootloader/tryboot; tryboot_hex=; if test -L "$tryboot_flag"; then exit 1; elif test -f "$tryboot_flag"; then tryboot_hex="$(od -An -tx1 -N4 "$tryboot_flag" | tr -d '[:space:]')"; fi; test "$tryboot_hex" != 00000001; test "$(lsmod | awk 'NR > 1 && $1 == "hyperpixel2r_kms" { count++ } END { print count+0 }')" = 1; for dependency in i2c_algo_bit edt_ft5x06 vc4; do test "$(lsmod | awk -v name="$dependency" 'NR > 1 && $1 == name { count++ } END { print count+0 }')" -ge 1; done; module_version=/sys/module/hyperpixel2r_kms/version; test ! -L "$module_version" && test -f "$module_version"; test "$(tr -d '\n' < "$module_version")" = "$1"; "#,
            r#"generic_driver=/sys/bus/platform/drivers/hyperpixel2r-kms; platform_devices=/sys/devices/platform; generic_bound_count=0; if test ! -L "$generic_driver" && test -d "$generic_driver"; then for entry in "$generic_driver"/*; do test -L "$entry" || continue; resolved_entry="$(readlink -f -- "$entry")" || continue; case "$resolved_entry" in "$platform_devices"/*) ;; *) continue;; esac; compatible="$resolved_entry/of_node/compatible"; test ! -L "$compatible" && test -f "$compatible" || continue; tr '\0' '\n' < "$compatible" | grep -Fxq shayne,hyperpixel2r-kms || continue; generic_bound_count=$((generic_bound_count + 1)); done; fi; test "$generic_bound_count" = 1; connected=0; for status in /sys/class/drm/card*-*/status; do test ! -L "$status" && test -f "$status" || continue; test "$(cat "$status")" = connected || continue; grep -Fxq 480x480 "$(dirname "$status")/modes" || continue; connected=$((connected + 1)); done; test "$connected" = 1; touch=false; for name in /sys/class/input/event*/device/name; do test ! -L "$name" && test -f "$name" || continue; if grep -Eiq 'EDT|FT5' "$name"; then touch=true; break; fi; done; test "$touch" = true"#,
        ),
