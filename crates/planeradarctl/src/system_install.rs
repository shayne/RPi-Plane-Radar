...
fn driver_receipt_command(
    ...
    RemoteCommand::interactive_sudo([
        ...
        "-c",
        concat!(
            r#"set -eu; root=/var/lib/hyperpixel2r-kms; state=$root/accepted-state; stock=$root/accepted-stock-config.txt; config=/boot/firmware/config.txt; "#,
            ...
            r#"if test "$5" = true; then test "$(uname -m)" = aarch64; test "$(uname -r)" = "$3"; tryboot_flag=/proc/device-tree/chosen/bootloader/tryboot; tryboot_hex=; if test -L "$tryboot_flag"; then exit 1; elif test -f "$tryboot_flag"; then tryboot_hex="$(od -An -tx1 -N4 "$tryboot_flag" | tr -d '[:space:]')"; fi; test "$tryboot_hex" != 00000001; test "$(lsmod | awk 'NR > 1 && $1 == "hyperpixel2r_kms" { count++ } END { print count+0 }')" = 1; for dependency in i2c_algo_bit edt_ft5x06 vc4; do test "$(lsmod | awk -v name="$dependency" 'NR > 1 && $1 == name { count++ } END { print count+0 }')" -ge 1; done; module_version=/sys/module/hyperpixel2r_kms/version; test ! -L "$module_version" && test -f "$module_version"; test "$(tr -d '\n' < "$module_version")" = "$1"; "#,
