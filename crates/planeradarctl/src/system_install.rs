...
use crate::driver::{
    ...
    GhDriverReleaseSource, GhDriverReleaseVerifier, LIFECYCLE_CAPABILITY,
    TargetProbe as DriverTargetProbe,
};
use crate::install::{
...
const BACKLIGHT_METADATA_SHELL: &str = r#"valid_id() { id=$1; case "$id" in 0|[1-9]*) ;; *) return 1;; esac; case "$id" in *[!0-9]*) return 1;; esac; test "$id" -le 4294967294; }; metadata_record() { record=$1; regular "$record" 600; test "$(stat -c '%s' -- "$record")" -le 160; test "$(awk -F= 'NF != 2 || $1 == "" || $2 == "" || seen[$1]++ { bad=1 } END { print NR ":" bad+0 }' "$record")" = "5:0"; metadata_value() { awk -F= -v key="$1" '$1 == key { print $2 }' "$record"; }; test "$(metadata_value schema_version)" = 1; present=$(metadata_value present); owner_uid=$(metadata_value owner_uid); group_gid=$(metadata_value group_gid); mode=$(metadata_value mode); case "$present" in true) valid_id "$owner_uid"; valid_id "$group_gid"; case "$mode" in [0-7][0-7][0-7]) ;; *) return 1;; esac;; false) test "$owner_uid:$group_gid:$mode" = none:none:none;; *) return 1;; esac; }; "#;

fn lifecycle_argument(capability: Option<&str>) -> Result<&str, TransportError> {
    match capability {
        None => Ok(""),
        Some(LIFECYCLE_CAPABILITY) => Ok(LIFECYCLE_CAPABILITY),
        Some(_) => Err(TransportError::InvalidRemoteCommand),
    }
}
...
pub fn staged_driver_transaction_command(
    ...
    let lifecycle = lifecycle_argument(expected.lifecycle_capability.as_deref())?;
    let program = [
        r#"set -eu; state=/var/lib/hyperpixel2r-kms/tryboot-state; regular() { test ! -L "$1" && test -f "$1" && test "$(stat -c '%u:%g:%a:%h' -- "$1")" = "0:0:$2:1"; }; boot_regular() { test ! -L "$1" && test -f "$1" && test "$(stat -c '%u:%g:%h' -- "$1")" = "0:0:1"; case "$(stat -c '%a' -- "$1")" in 644|755) ;; *) return 1;; esac; }; digest() { test "$(sha256sum -- "$1" | awk '{print $1}')" = "$2"; }; absent() { test ! -L "$1" && test ! -e "$1"; }; valid_sha() { candidate_sha=$1; case "$candidate_sha" in *[!0-9a-f]*|'') return 1;; esac; test "${#candidate_sha}" = 64; }; "#,
        BACKLIGHT_METADATA_SHELL,
        r#"lifecycle=${17}; case "$lifecycle" in '') state_rows=23; manifest_rows=17; manifest_schema=2;; exact-backlight-metadata-v1) state_rows=25; manifest_rows=18; manifest_schema=3;; *) exit 1;; esac; regular "$state" 600; value() { awk -F= -v key="$1" '$1 == key { print $2 }' "$state"; }; test "$(value schema_version)" = 4; test "$(awk -F= 'NF != 2 || $1 == "" || $2 == "" || seen[$1]++ { bad=1 } END { print NR ":" bad+0 }' "$state")" = "$state_rows:0"; test "$(value driver_version)" = "$1"; test "$(value source_revision)" = "$2"; test "$(value source_tree)" = "$3"; test "$(value kernel_release)" = "$4"; test "$(value module_file)" = "$7"; test "$(value module_sha256)" = "$8"; test "$(value overlay_file)" = "$9"; test "$(value overlay_sha256)" = "${10}"; test "$(value applied_dtb_file)" = "${11}"; test "$(value applied_dtb_sha256)" = "${12}"; test "$(value replaced_overlay)" = "${13}"; test "$(value backlight_rule_file)" = "${15}"; test "$(value backlight_rule_sha256)" = "${16}"; case "$(value module_existed)" in true|false) ;; *) exit 1;; esac; case "$(value overlay_existed)" in true|false) ;; *) exit 1;; esac; valid_sha "$(value prior_dkms_inventory_sha256)"; prior=$(value prior_tryboot_sha256); case "$(value tryboot_existed)" in true) valid_sha "$prior";; false) test "$prior" = none;; *) exit 1;; esac; for key in normal_config_sha256 candidate_config_sha256; do valid_sha "$(value "$key")"; done; artifact="/usr/lib/hyperpixel2r-kms/$1/$2/$4"; test ! -L "$artifact" && test -d "$artifact" && test "$(stat -c '%u:%g:%a' -- "$artifact")" = "0:0:755"; case "$(value prior_backlight_rule_existed)" in true) valid_sha "$(value prior_backlight_rule_sha256)"; regular "$artifact/prior-backlight-rule" 600; digest "$artifact/prior-backlight-rule" "$(value prior_backlight_rule_sha256)";; false) test "$(value prior_backlight_rule_sha256)" = none; absent "$artifact/prior-backlight-rule";; *) exit 1;; esac; metadata="$artifact/prior-backlight-metadata"; if test -n "$lifecycle"; then test "$(value backlight_metadata_capability)" = "$lifecycle"; valid_sha "$(value prior_backlight_metadata_sha256)"; metadata_record "$metadata"; digest "$metadata" "$(value prior_backlight_metadata_sha256)"; else absent "$metadata"; fi; manifest="$artifact/manifest.txt"; regular "$manifest" 644 && digest "$manifest" "$6"; field() { awk -F '\t' -v key="$1" '$1 == key { print $2 }' "$manifest"; }; test "$(awk -F '\t' 'NF != 2 || $1 == "" || $2 == "" || seen[$1]++ { bad=1 } END { print NR ":" bad+0 }' "$manifest")" = "$manifest_rows:0"; test "$(field schema_version)" = "$manifest_schema"; test "$(field driver_version)" = "$1"; test "$(field source_revision)" = "$2"; test "$(field source_tree)" = "$3"; test "$(field kernel_release)" = "$4"; test "$(field capability)" = "${14}"; test "$(field module_vermagic)" = "$5"; test "$(field module_file)" = "$7"; test "$(field module_sha256)" = "$8"; test "$(field overlay_file)" = "$9"; test "$(field overlay_sha256)" = "${10}"; test "$(field applied_dtb_file)" = "${11}"; test "$(field applied_dtb_sha256)" = "${12}"; test "$(field backlight_rule_file)" = "${15}"; test "$(field backlight_rule_sha256)" = "${16}"; if test -n "$lifecycle"; then test "$(field lifecycle_capability)" = "$lifecycle"; fi; regular "$artifact/$7" 644 && digest "$artifact/$7" "$8"; regular "$artifact/$9" 644 && digest "$artifact/$9" "${10}"; regular "$artifact/${11}" 644 && digest "$artifact/${11}" "${12}"; regular "$artifact/${15}" 644 && digest "$artifact/${15}" "${16}"; module="/lib/modules/$4/extra/$7"; overlay="/boot/firmware/overlays/$9"; rule="/etc/udev/rules.d/${15}"; normal=/boot/firmware/config.txt; candidate=/boot/firmware/tryboot.txt; regular "$module" 644 && digest "$module" "$8"; boot_regular "$overlay" && digest "$overlay" "${10}"; regular "$rule" 644 && digest "$rule" "${16}"; boot_regular "$normal" && digest "$normal" "$(value normal_config_sha256)"; boot_regular "$candidate" && digest "$candidate" "$(value candidate_config_sha256)"; test "$(awk -v wanted="dtoverlay=$9" '{ line=$0; sub(/^[[:space:]]+/, "", line); sub(/[[:space:]]+$/, "", line); if (line !~ /^dtoverlay=/) next; if (line == wanted) { count++; next } if (line ~ /hyperpixel2r/) bad=1 } END { print count ":" bad+0 }' "$candidate")" = "1:0""#,
    ]
    .concat();
    RemoteCommand::interactive_sudo(vec![
        "sudo".to_owned(),
        "sh".to_owned(),
        "-c".to_owned(),
        program,
        "planeradar-driver-transaction".to_owned(),
        expected.driver_version.clone(),
        expected.source_revision.clone(),
        expected.source_tree.clone(),
        expected.kernel_release.clone(),
        expected.module_vermagic.clone(),
        expected.manifest_sha256.clone(),
        expected.module_file.clone(),
        expected.module_sha256.clone(),
        expected.overlay_file.clone(),
        expected.overlay_sha256.clone(),
        expected.applied_dtb_file.clone(),
        expected.applied_dtb_sha256.clone(),
        replaced_overlay.to_owned(),
        expected.capability.clone(),
        expected.backlight_rule_file.clone(),
        expected.backlight_rule_sha256.clone(),
        lifecycle.to_owned(),
    ])
}
...
fn driver_receipt_command(
    ...
    let program = [
        ...
        r#"if test "$8" = true; then test "$(uname -m)" = aarch64; test "$(uname -r)" = "$3"; tryboot_flag=/proc/device-tree/chosen/bootloader/tryboot; tryboot_hex=; if test -L "$tryboot_flag"; then exit 1; elif test -f "$tryboot_flag"; then tryboot_hex="$(od -An -tx1 -N4 "$tryboot_flag" | tr -d '[:space:]')"; fi; test "$tryboot_hex" != 00000001; test "$(lsmod | awk 'NR > 1 && $1 == "hyperpixel2r_kms" { count++ } END { print count+0 }')" = 1; for dependency in i2c_algo_bit edt_ft5x06 vc4; do test "$(lsmod | awk -v name="$dependency" 'NR > 1 && $1 == name { count++ } END { print count+0 }')" -ge 1; done; module_version=/sys/module/hyperpixel2r_kms/version; test ! -L "$module_version" && test -f "$module_version"; test "$(tr -d '\n' < "$module_version")" = "$1"; backlight=/sys/class/backlight/hyperpixel2r-backlight; test ! -L "$backlight/max_brightness" && test -f "$backlight/max_brightness"; case "$(cat "$backlight/max_brightness")" in ''|*[!0-9]*|0) exit 1;; esac; "#,
    ...
    RemoteCommand::interactive_sudo(vec![
        ...
        "-c".to_owned(),
        program,
        "planeradar-driver-accepted-reuse".to_owned(),
        expected.driver_version.clone(),
        expected.source_revision.clone(),
        expected.kernel_release.clone(),
        expected.overlay_file.clone(),
        expected.required_capability.clone(),
        expected.backlight_rule_file.clone(),
        lifecycle.to_owned(),
        if require_normal_boot { "true" } else { "false" }.to_owned(),
    ])
}
...
pub fn committed_driver_command(
    ...
    let lifecycle = lifecycle_argument(expected.lifecycle_capability.as_deref())?;
    let program = [
        r#"set -eu; state=/var/lib/hyperpixel2r-kms/tryboot-state; regular() { test ! -L "$1" && test -f "$1" && test "$(stat -c '%u:%g:%a:%h' -- "$1")" = "0:0:$2:1"; }; boot_regular() { test ! -L "$1" && test -f "$1" && test "$(stat -c '%u:%g:%h' -- "$1")" = "0:0:1"; case "$(stat -c '%a' -- "$1")" in 644|755) ;; *) return 1;; esac; }; digest() { test "$(sha256sum -- "$1" | awk '{print $1}')" = "$2"; }; absent() { test ! -L "$1" && test ! -e "$1"; }; "#,
        BACKLIGHT_METADATA_SHELL,
        r#"lifecycle=${17}; case "$lifecycle" in '') manifest_rows=17; manifest_schema=2;; exact-backlight-metadata-v1) manifest_rows=18; manifest_schema=3;; *) exit 1;; esac; test ! -L "$state" && test ! -e "$state"; artifact="/usr/lib/hyperpixel2r-kms/$1/$2/$4"; test ! -L "$artifact" && test -d "$artifact" && test "$(stat -c '%u:%g:%a' -- "$artifact")" = "0:0:755"; manifest="$artifact/manifest.txt"; regular "$manifest" 644 && digest "$manifest" "$6"; field() { awk -F '\t' -v key="$1" '$1 == key { print $2 }' "$manifest"; }; test "$(awk -F '\t' 'NF != 2 || $1 == "" || $2 == "" || seen[$1]++ { bad=1 } END { print NR ":" bad+0 }' "$manifest")" = "$manifest_rows:0"; test "$(field schema_version)" = "$manifest_schema"; test "$(field driver_version)" = "$1"; test "$(field source_revision)" = "$2"; test "$(field source_tree)" = "$3"; test "$(field kernel_release)" = "$4"; test "$(field capability)" = "${14}"; test "$(field module_vermagic)" = "$5"; test "$(field module_file)" = "$7"; test "$(field module_sha256)" = "$8"; test "$(field overlay_file)" = "$9"; test "$(field overlay_sha256)" = "${10}"; test "$(field applied_dtb_file)" = "${11}"; test "$(field applied_dtb_sha256)" = "${12}"; test "$(field backlight_rule_file)" = "${15}"; test "$(field backlight_rule_sha256)" = "${16}"; metadata="$artifact/prior-backlight-metadata"; if test -n "$lifecycle"; then test "$(field lifecycle_capability)" = "$lifecycle"; metadata_record "$metadata"; else absent "$metadata"; fi; regular "$artifact/$7" 644 && digest "$artifact/$7" "$8"; regular "$artifact/$9" 644 && digest "$artifact/$9" "${10}"; regular "$artifact/${11}" 644 && digest "$artifact/${11}" "${12}"; regular "$artifact/${15}" 644 && digest "$artifact/${15}" "${16}"; module="/lib/modules/$4/extra/$7"; overlay="/boot/firmware/overlays/$9"; rule="/etc/udev/rules.d/${15}"; config=/boot/firmware/config.txt; regular "$module" 644 && digest "$module" "$8"; boot_regular "$overlay" && digest "$overlay" "${10}"; regular "$rule" 644 && digest "$rule" "${16}"; boot_regular "$config"; test "$(awk -v wanted="dtoverlay=$9" '{ line=$0; sub(/^[[:space:]]+/, "", line); sub(/[[:space:]]+$/, "", line); if (line !~ /^dtoverlay=/) next; if (line == wanted) { count++; next } if (line ~ /hyperpixel2r/) bad=1 } END { print count ":" bad+0 }' "$config")" = "1:0""#,
    ]
    .concat();
    RemoteCommand::interactive_sudo(vec![
        "sudo".to_owned(),
        ...
        "-c".to_owned(),
        program,
        "planeradar-driver-committed".to_owned(),
        expected.driver_version.clone(),
        expected.source_revision.clone(),
        expected.source_tree.clone(),
        expected.kernel_release.clone(),
        ...
        expected.applied_dtb_file.clone(),
        expected.applied_dtb_sha256.clone(),
        expected.replaced_overlay.clone(),
        expected.capability.clone(),
        expected.backlight_rule_file.clone(),
        expected.backlight_rule_sha256.clone(),
        lifecycle.to_owned(),
