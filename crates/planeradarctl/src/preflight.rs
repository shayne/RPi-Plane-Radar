...
pub const TARGET_FACTS_SCRIPT: &str = concat!(
    ...
    r#"overlay_lines=$(awk '{ line=$0; sub(/^[[:space:]]+/, "", line); sub(/[[:space:]]+$/, "", line); if (line ~ /^dtoverlay=/) print line }' "$boot_config"); external_hyperpixel_overlay_count=$(printf '%s\n' "$overlay_lines" | grep -Ec '^dtoverlay=hyperpixel2r-kms-[0-9a-f]{12}\.dtbo$' || true); hyperpixel_declaration_count=$(printf '%s\n' "$overlay_lines" | grep -Ec '^dtoverlay=(vc4-kms-dpi-hyperpixel2r|planeradar-hyperpixel2r-[0-9a-f]{12}|hyperpixel2r-kms-[0-9a-f]{12}\.dtbo)$' || true); unsafe_overlay_present=$(if printf '%s\n' "$overlay_lines" | grep -Ev '^(|dtoverlay=vc4-kms-v3d|dtoverlay=vc4-kms-dpi-hyperpixel2r|dtoverlay=planeradar-hyperpixel2r-[0-9a-f]{12}|dtoverlay=hyperpixel2r-kms-[0-9a-f]{12}\.dtbo)$' | grep -q . || test "$hyperpixel_declaration_count" -gt 1; then printf true; else printf false; fi); "#,
